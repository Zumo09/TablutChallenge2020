...
public class BrainDeepGen extends Brain {

	private KernelLSTM kernel;
	...
	public BrainDeepGen(int timeout, int gametype, double mutationProb, double mutationScale, int depth) {
		...
		this.kernel = new KernelLSTM(mutationProb, mutationScale, depth);
	...
	public BrainDeepGen(int timeout, int gametype) {
		// The constructor for the runtime player
		super(timeout, gametype);
		this.kernel = KernelLSTM.load();
		this.actions = new TreeMap<>();
	}

	public KernelLSTM getKernel() {
		return kernel;
	}

	public void setKernel(KernelLSTM kernel) {
		this.kernel = kernel;
	}
